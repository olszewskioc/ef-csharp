...
namespace Entity_Forms.Services
{
    public class CrudMaquina
    {
        ...
        public List<string>? Listar(int? id = null, string? tipo = null, int? velocidade = null, int? hd = null, int? rede = null, int? ram = null, int? usuario = null)
        {
            try
            {
                ...
                using (var db = new AppDbContext())
                {
                    ...
                    {
                        result.Add($"ID: {maquina.MaquinaId} | TIPO: {maquina.Tipo} | VELOCIDADE: {maquina.Velocidade} | HD: {maquina.HardDisk} | RAM: {maquina.MemoriaRam} | REDE: {maquina.PlacaRede} | USER: {maquina.UsuarioId}");
                    }
                    return result;
                }
            }
